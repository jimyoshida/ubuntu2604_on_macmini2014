...
The versioned directory plus a symlink is the [`maven.yml`](#mavenyml)/[`zap.yml`](#zapyml)
shape: bumping `sonar_scanner_version` installs beside the old tree and moves the symlink, and
`ls -l /usr/local/bin/sonar-scanner` says which version is active. The role this replaces
...
[`maven.yml`](#mavenyml), [`zap.yml`](#zapyml) and
[`cloud-cli/jenkins-cli.yml`](../cloud-cli/README.md#jenkins-cliyml), this playbook therefore
...
[`dotnet-tools.yml`](#dotnet-toolsyml) installs: that one is the MSBuild-integrated scanner for
.NET projects, this one is the standalone CLI for everything else.
...
```bash
ansible-playbook misc/sonar-scanner.yml -e host=ws01 -e sonar_scanner_version=8.2.0.1234
ansible-playbook misc/sonar-scanner.yml -e host=ws01 -e sonar_scanner_host_url=https://sonar.example.com
```
